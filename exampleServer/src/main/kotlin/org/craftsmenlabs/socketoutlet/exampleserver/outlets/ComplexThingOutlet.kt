...
class ComplexThingOutlet : Outlet<ComplexThing>(ComplexThing::class.java) {
    ...
    override fun onMessage(sender: String, message: ComplexThing, egress: Egress) {
        logger.i { "Just received a ComplexThing: $message from $sender" }
        egress.send(message.simpleThing)
        logger.i { "And replied with the SimpleThing: ${message.simpleThing}" }
    }
}
